...
export default function InvestigationClient({ id }: { id: string }) {
  ...
  async function resetScenario() {
    try {
      ...
      setMemory(null);
      setStage("live");
      setNotice("Scenario reset. 37 robots are known affected at decision time.");
      await refresh();
    } catch (error) {
  ...
  async function lateEvidence() {
    try {
      ...
      await refresh();
      setStage("late");
      setNotice("Delayed edge-buffer evidence arrived with event_time before the customer ticket.");
    } catch (error) {
  ...
  const lateEvidenceVisible = affected > 37;
  const detectionLead = useMemo(() => {
    if (!rec?.human_discovery?.event_time || !rec?.first_abnormal_evidence?.event_time) return "before customer report";
    ...
    return `${mins} minutes before the customer ticket`;
  }, [rec]);
    ...
    <div className="shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          ...
          <button className={stage === "live" ? "active" : ""} onClick={() => setStage("live")}>Live failure</button>
          ...
          <button className={stage === "memory" ? "active" : ""} onClick={() => setStage("memory")}>47 days later</button>
      ...
      <main className="main">
        <div className="topbar">
          <span className="eyebrow">120 Robots · One Bad Rollout · 90 Seconds</span>
        ...
        <section className="hero demo-hero">
          <span className="eyebrow">Morning after deployment</span>
          <h1>The rollout that changed 37 robots.</h1>
        ...
        <div className="status-strip">
          ...
          {lateEvidenceVisible && <b>37 decision-time</b>}
          <b>{healthy} healthy</b>
          <b>120 updated</b>
        </div>
    ...
    <>
      <div className="fleet-map" aria-label="fleet status">
        {Array.from({ length: 120 }).map((_, i) => {
          const affected = i < (comparison?.same_signal ?? 37);
          return <span key={i} className={affected ? "dot bad" : "dot good"} title={`AMR-${String(i + 1).padStart(3, "0")}`} />;
        })}
      </div>
      <div className="grid four">
        <Metric label="14:02:11" value="Deploy v2.4" note="rollout started" />
        <Metric label="14:04:37" value="120/120" note="updated" />
        <Metric label="14:11:08" value="First signal" note={rec?.first_abnormal_evidence?.payload?.signal || "navigation degradation"} />
        <Metric label="14:18:42" value="37 affected" note="pattern detected 7m 21s before ticket" />
        <Metric label="14:26:03" value="Ticket arrives" note="customer reported it" />
      </div>
      <article className="panel dramatic">
        <span className="eyebrow">7 minutes and 21 seconds before the customer ticket</span>
        <h2>Software changed everywhere. The failure did not.</h2>
        <p>That is how long customer trust is exposed when the fleet knows something before the support team does. Every robot received application v2.4. Only a subset began showing abnormal navigation behavior.</p>
...
function CompareStage({ comparison, onPackage }: any) {
  return (
    <article className="panel">
      ...
      <h2>Same software. Same deployment. Why does only one-third fail?</h2>
      <div className="grid three">
        <Metric label="Same change" value={`${comparison?.same_change ?? 120}`} note="robots on application v2.4" />
        <Metric label="Same signal" value={`${comparison?.same_signal ?? 37}`} note="navigation degradation detected" />
        <Metric label="No signal" value={`${comparison?.no_signal ?? 83}`} note="updated but healthy" />
      </div>
      ...
      <div className="callout">
        ...
        <p>Application v2.4 is shared across both groups. Warehouse-east config, motor firmware 7.2 and Private 5G are plausible but imperfect. Network Profile C contains all known affected robots plus exposed peers to watch. This narrows the investigation; it does not establish cause.</p>
      </div>
  ...
  return (
    <article className="panel">
      ...
      <div className="package-grid">
        <PackageItem title="Trigger" value="navigation degradation after rollout" />
        <PackageItem title="Last known healthy" value={short(rec?.last_known_healthy?.event_time)} />
        <PackageItem title="Recent changes" value="app 2.3 → 2.4 · profile A → C" />
        <PackageItem title="Machine state" value={rec?.current_state?.health || "degraded"} />
        <PackageItem title="Affected vs healthy" value={`${comparison?.same_signal ?? 37} / ${comparison?.same_change ?? 120}`} />
        <PackageItem title="Observed" value="customer ticket received at 14:26" />
        <PackageItem title="Inferred" value="Profile C is the highest-priority lead" />
        <PackageItem title="Human asserted" value="engineer suspects site-side network instability" />
        <PackageItem title="Missing evidence" value={(pkg.package.missing_evidence || []).join(" · ")} />
        <PackageItem title="Human action" value={pkg.sealed ? "Pause rollout on Profile C sites" : "not recorded yet"} />
        <PackageItem title="Outcome" value="pending" />
      </div>
      <div className="callout">
        <b>What this package rules in / rules out</b>
        <p>Deployment-wide issue: not supported by current peer comparison. Profile-specific issue: plausible. Site/network contribution: still unresolved. Field intervention: current evidence is insufficient to support it.</p>
      </div>
...
function LateEvidenceStage({ pkg, comparison, onSeal, onOutcome }: any) {
  return (
    <article className="panel dramatic">
      ...
      <h2>Late evidence reveals AMR-038 had already shown the signal at 14:09.</h2>
      ...
      <div className="grid three">
        <Metric label="Decision-time view" value="37" note="affected in sealed package" />
        <Metric label="Current view" value={`${comparison?.same_signal ?? 38}`} note="affected after delayed evidence" />
...
function MemoryStage({ memory, onOutcome }: any) {
  ...
  return (
    <article className="panel final-stage">
      <span className="eyebrow">47 days later</span>
      <h2>{hasMemory ? "Similar operational pattern found." : "No prior outcome recorded yet."}</h2>
      {hasMemory ? (
        <>
          <p>AMR-218 begins showing abnormal navigation after deployment.</p>
          <div className="callout">
            <b>Similar operational pattern found</b>
            <p>Previous incident: 47 days ago. Shared pattern: deployment + Profile C + navigation degradation. Same pattern: one network-profile cluster diverges after deployment. Previous action: isolate rollout group. Outcome: recovered in 18 minutes. Different this time: motor firmware 7.3 on the affected subset.</p>
          </div>
          <div className="ending">
            <b>The first incident took 42 minutes to understand.</b>
            <b>The second took 42 seconds.</b>
            <span>The first time you understand an incident, you earn the right to understand the next one in seconds.</span>
          </div>
        </>
      ) : (
