...
function MemoryScene({ memoryReady, onNext }: { memoryReady: boolean; onNext: () => void }) {
  return <section className="cinema-scene center"><span className="eyebrow">Scene 6 · Operational Memory</span><h1>12 days later, the next run starts with memory.</h1><div className="memory-card"><p><b>Operational Memory match found</b></p><p>Similar change pattern: policy update + calibration C + grip pose drift.</p><p>Same pattern: one calibration/firmware subset diverges while similarly updated robots stay stable.</p><p>Previous human action: pause v0.9 on robots with calibration C and firmware 7.3.</p><p>Previous outcome: recovered after targeted rollback.</p><p>Missing last time: targeted low-light validation runs.</p><p>Different this time: low-light bin appears on the affected run.</p></div><div className="time-saved"><b>The first run took 42 minutes.</b><b>The next run took 42 seconds.</b></div><div className="future-strip"><span>What this unlocks</span><b>Better experiments · Faster pilot review · Operational memory</b></div><button className="button lime" onClick={onNext}>{memoryReady ? "End" : "End"}</button></section>;
}

function EndScene() {
  return <section className="cinema-scene center end-frame"><h1>Today: six robots in a lab. Tomorrow: six hundred in the field. The memory should start now.</h1><p>Veyra — Operational context for Physical AI.</p><div className="unlock-row"><div><span>Better experiments</span><b>Faster review</b></div><div><span>Pilot readiness</span><b>Watch exposed runs</b></div><div><span>Operational memory</span><b>Reuse what worked</b></div></div><Link className="button lime" href={`/investigations/${INVESTIGATION_ID}`}>Open live system</Link></section>;
}
