...
export default function CinematicDemo() {
  ...
  async function ensurePackage() {
    ...
    await postJson(`/investigations/${INVESTIGATION_ID}/decision`, {
      decision: "Pause policy v0.9 on calibration C plus gripper firmware 7.3 robots",
...
function CompareScene({ comparison, onNext }: { comparison: Comparison | null; onNext: () => void }) {
  ...
  return <section className="cinema-scene"><SceneTitle eyebrow="Scene 3 · Compare" title="Same model. Same task. Why do only two robots diverge?" subtitle="This is the question that matters." /><div className="split-count"><div><strong>{comparison?.same_signal ?? 2}</strong><span>affected</span></div><div><strong>{comparison?.no_signal ?? 4}</strong><span>stable</span></div></div><table className="table cinema-table"><thead><tr><th>Context</th><th>Affected</th><th>Stable</th></tr></thead><tbody>{rows.map((row) => <tr className={row.context === "Camera calibration C" || row.context === "Gripper firmware 7.3" ? "highlight-row" : ""} key={row.context}><td>{row.context}</td><td>{row.affected}</td><td>{row.unaffected}</td></tr>)}</tbody></table><HeroLine text="One lead rises above the noise: calibration C plus gripper firmware 7.3." /><p className="scene-footnote">This narrows the investigation and identifies one exposed robot to watch. It does not establish cause.</p><button className="button primary" onClick={onNext}>Generate Decision Package</button></section>;
}

function DecisionScene({ pkg, onNext }: { pkg: DecisionPackage | null; onNext: () => void }) {
  return <section className="cinema-scene"><SceneTitle eyebrow="Scene 4 · Decision Package" title="A decision made during one run should not be rewritable after the next log arrives." subtitle="This package locks what the team knew at the moment they knew it." /><div className="decision-card"><div><span>Observed</span><b>2 robots affected</b></div><div><span>Observed</span><b>policy v0.9 ran on 6</b></div><div><span>Observed</span><b>engineer note at 14:26</b></div><div><span>Inferred</span><b>calibration C + firmware 7.3 is the highest-priority lead</b></div><div><span>Human asserted</span><b>engineer suspects calibration mismatch</b></div><div><span>Missing evidence</span><b>targeted low-light demonstrations</b></div><div><span>Last known healthy</span><b>14:02</b></div><div><span>Human decision</span><b>pause policy on calibration C + firmware 7.3</b></div><div><span>Outcome</span><b>pending</b></div></div><div className="seal-card"><span>Sealed Decision Package · 14:27</span><b>{pkg?.digest ? pkg.digest.slice(0, 18) + "..." : "SHA-256 pending"}</b><small>12 evidence items · 6 robots compared · human decision recorded</small></div><button className="button primary" onClick={onNext}>Inject delayed evidence</button></section>;
}

function LateScene({ comparison, pkg, onNext }: { comparison: Comparison | null; pkg: DecisionPackage | null; onNext: () => void }) {
  return <section className="cinema-scene"><SceneTitle eyebrow="Scene 5" title="Late evidence reveals R06 had already shown the signal at 14:09." subtitle="This happened before the decision. The team learned about it after." /><div className="time-rail cinematic-rail"><div><b>14:09</b><span>event_time</span><small>R06 was already affected</small></div><i /><div><b>14:27</b><span>decision sealed</span><small>2 affected in the package</small></div><i /><div><b>14:31</b><span>known_at</span><small>late evidence becomes knowable</small></div><i /><div><b>14:31:04</b><span>ingested_at</span><small>received by Veyra</small></div></div><div className="late-grid"><div><span>Decision-time view</span><b>2 affected</b></div><div><span>Current view</span><b>{comparison?.same_signal ?? 3} affected</b></div><div><span>Sealed package</span><b>{pkg?.sealed ? "unchanged" : "decision-time snapshot"}</b></div></div><HeroLine text="Veyra updates what we know now, not what the team knew then." /><button className="button primary" onClick={onNext}>12 days later</button></section>;
}

function MemoryScene({ memoryReady, onNext }: { memoryReady: boolean; onNext: () => void }) {
  return <section className="cinema-scene center"><span className="eyebrow">Scene 6 · Operational Memory</span><h1>12 days later, the next run starts with memory.</h1><div className="memory-card"><p><b>Operational Memory match found</b></p><p>Similar change pattern: policy update + calibration C + grip pose drift.</p><p>Same pattern: one calibration/firmware subset diverges while similarly updated robots stay stable.</p><p>Previous human action: pause policy on calibration C + firmware 7.3.</p><p>Previous outcome: recovered after targeted rollback.</p><p>Missing last time: targeted low-light demonstrations.</p><p>Different this time: low-light bin appears on the affected run.</p></div><div className="time-saved"><b>The first run took 42 minutes.</b><b>The next run took 42 seconds.</b></div><div className="future-strip"><span>What this unlocks</span><b>Better experiments · Faster pilot review · Operational memory · Machine economics</b></div><button className="button lime" onClick={onNext}>{memoryReady ? "End" : "End"}</button></section>;
}

function EndScene() {
  return <section className="cinema-scene center end-frame"><h1>Today: six robots in a lab. Tomorrow: six hundred in the field. The memory should start now.</h1><p>Veyra — Operational context for Physical AI.</p><div className="unlock-row"><div><span>Better experiments</span><b>Faster review</b></div><div><span>Pilot readiness</span><b>Watch exposed runs</b></div><div><span>Operational memory</span><b>Reuse what worked</b></div><div><span>Machine economics</span><b>Evidence for maintenance, insurance and usage-based financing</b></div></div><p className="future-line">Reliable operational history can become the evidence behind maintenance, insurance, performance guarantees and usage-based financing.</p><Link className="button lime" href={`/investigations/${INVESTIGATION_ID}`}>Open live system</Link></section>;
}
